...
public final class WandSettingsState extends SavedData {
    ...
    public static final SavedData.Factory<WandSettingsState> TYPE = new SavedData.Factory<>(
            WandSettingsState::new,
            (tag, registries) -> CODEC.parse(NbtOps.INSTANCE, tag).result().orElseGet(WandSettingsState::new),
            DataFixTypes.LEVEL
    );
    ...
    public static WandSettingsState get(MinecraftServer server) {
        return server.overworld().getDataStorage().computeIfAbsent(TYPE, SAVE_ID);
    }
