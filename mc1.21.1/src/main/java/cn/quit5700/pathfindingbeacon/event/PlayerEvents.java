...
public final class PlayerEvents {
    ...
    public static void register() {
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            handler.player.displayClientMessage(Component.translatable("text.quick_build.152"), false);
    ...
    private static void unlockRecipesForPickaxeOwners(MinecraftServer server) {
        ...
        ids.add(recipeKey("route_block_1").location());
        for (int i = 2; i <= 30; i++) {
            ids.add(recipeKey("route_block_" + i).location());
        }
        ids.add(recipeKey("id_sequence_reorderer").location());
