...
public final class ClientWandController {
    ...
    private static void tick(Minecraft client) {
        ...
        if (moveWand && !wasHoldingMoveWand) {
            ...
            ClientPlayNetworking.send(WandNetworking.QUERY_MOVE_MEMORY, WandNetworking.buffer(new WandNetworking.QueryMoveMemoryPayload()));
        }
    ...
    private static void tickBuildingWand(
        ...
        if (pendingBuilding != null) {
            ...
            if (attackPressed && !alt) {
                ClientPlayNetworking.send(WandNetworking.PLACE_SHAPE, WandNetworking.buffer(new WandNetworking.PlaceShapePayload(
        ...
        if (attackPressed && alt) {
            if (hit instanceof BlockHitResult blockHit && hit.getType() == HitResult.Type.BLOCK) {
                ...
                if (!state.isAir()) {
                    ...
                    ClientPlayNetworking.send(WandNetworking.SELECT_BLOCK, WandNetworking.buffer(new WandNetworking.SelectBlockPayload(selectedPos)));
                }
            }
            buildingDrawing = null;
            return;
    ...
    private static void finishBuildingGesture(Minecraft client, boolean shift, boolean ctrl) {
        ...
        ClientPlayNetworking.send(WandNetworking.PLACE_SINGLE, WandNetworking.buffer(new WandNetworking.PlaceSinglePayload(drawing.current())));
    }
    ...
    private static void tickMoveWand(
        ...
        if (usePressed && shift) {
            ClientPlayNetworking.send(WandNetworking.CANCEL_MOVE_SELECTION, WandNetworking.buffer(new WandNetworking.CancelMoveSelectionPayload()));
            resetMoveClientState();
            return;
        }
        ...
        switch (MoveWandFlow.leftAction(moveStage, hasMoveMemory, pendingMove != null)) {
            ...
            case EXECUTE_MOVE -> {
                moveStage = Stage.EXECUTE_PENDING;
                ClientPlayNetworking.send(WandNetworking.EXECUTE_MOVE, WandNetworking.buffer(new WandNetworking.ExecuteMovePayload(
